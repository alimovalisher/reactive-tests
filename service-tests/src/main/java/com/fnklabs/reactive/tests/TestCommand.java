package com.fnklabs.reactive.tests;

import com.google.common.net.HostAndPort;
import com.google.common.util.concurrent.ThreadFactoryBuilder;
import com.fnklabs.reactive.tests.core.Feed;
import com.fnklabs.reactive.tests.core.Message;
...
public class TestCommand {
    ...
    private final AtomicReference<Disposable> activeScheduleTask = new AtomicReference<>();
    ...
    public void stop() {
        scheduleStopWithDelay(null);
        addNewContext(null);

        log.info("stopped");
    }

    private void scheduleStopWithDelay(Disposable schedule) {
        activeScheduleTask.getAndUpdate(existing -> {
            if (existing != null) {
                existing.dispose();
            }
            return schedule;
    ...
    @ShellMethod(key = "messages:bootstrap", value = "create messages")
    public void create(
        ...
        Flux<Message> generateMsgFlux = Flux.<Message>create(sink -> {
            for (int j = 0; j < messagesBatchSize; j++) {
                sink.next(feed.get());
            }
            sink.complete();
        });
        ...
        Flux<Void> sendMsgFlux = switch (type) {
            ...
            case TEST_TYPE_FLUX -> generateMsgFlux.flatMap(msg -> {
                        ...
                        return webClient.post()
                                ...
                                .body(Flux.just(msg), Message.class)
                                .retrieve()
                                .bodyToMono(Void.class)
                                ;
                    }
            ...
            case TEST_TYPE_RSOCKET -> {
                ...
                yield generateMsgFlux.flatMap(msg -> {
                    ...
                    return requester.route("messages.add")
                            .data(Flux.just(msg), Message.class)
        ...
        Disposable subscribe = sendMsgFlux.then()
